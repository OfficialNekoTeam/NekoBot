...
def test_in_memory_store_save_get_and_list() -> None:
    ...
    saved = store.save(context)
    loaded = store.get_conversation(conversation_key.value)
    ...
    assert store.list_conversation_keys() == (conversation_key.value,)


def test_in_memory_store_returns_cloned_contexts() -> None:
    store = InMemoryConversationStore()
    context = _conversation_context()
    conversation_key = context.conversation_key
    ...
    _ = store.save(context)

    loaded = store.get_conversation(conversation_key.value)
    assert loaded is not None
    loaded.history.append({"role": "assistant", "content": "mutated"})

    reloaded = store.get_conversation(conversation_key.value)
    assert reloaded is not None
    assert reloaded.history == [{"role": "user", "content": "hello"}]


def test_in_memory_store_delete_removes_conversation_and_session() -> None:
    store = InMemoryConversationStore()
    context = _conversation_context()
    conversation_key = context.conversation_key
    ...
    store.delete(conversation_key.value)

    assert store.get_conversation(conversation_key.value) is None
    assert store.get_session(session_key.value) is None


def test_sqlite_store_persists_and_loads_context(tmp_path: Path) -> None:
...
def test_sqlite_store_lists_and_deletes_conversations(tmp_path: Path) -> None:
    store = SQLiteConversationStore(tmp_path / "conversations.sqlite3")
    context = _conversation_context()
    conversation_key = context.conversation_key
    ...
    assert store.list_conversation_keys() == ()
    assert store.get_conversation(conversation_key.value) is None
