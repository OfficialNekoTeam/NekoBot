...
class StreamableHTTPMCPClient:
    def __init__(self, config: MCPServerConfig) -> None:
        if not config.url:
            raise ValueError(f"MCP streamable-http {config.name!r} requires 'url'")
    ...
    async def start(self) -> None:
        ...
        logger.info("MCP http[{}]: started ({} tool(s))", self.server_name, len(self._tools))

    async def stop(self) -> None:
        await self._exit_stack.aclose()
        self._session = None
        logger.info("MCP http[{}]: stopped", self.server_name)
